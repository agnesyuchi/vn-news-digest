...
# 以下九項原則是分類（CATEGORY_RULES）與最終重要性排序（build_final_dedupe_prompt）
# 共用的核心判斷框架，避免兩處各自維護一份規則、日後修改時彼此不一致。
# 原則編號（一～九）本身即代表優先順序，編號越前面優先程度越高。
PRIORITY_PRINCIPLES = """
【原則一：雙邊與多邊外交事務】
凡涉及越南或寮國與他國之間的高層互動、聯合聲明、協議簽署、禮儀性往來（賀電、
唁電、弔唁）等，均應列入；涉及東協、湄公河次區域合作機制、瀾湄合作等區域架構
中越南或寮國的角色與立場，亦應列入。

【原則二：越寮特殊關係與跨境治理】
越南與寮國具有歷史悠久的「特殊關係」，兩國高層互訪、邊境劃界與管理、越柬老
三國司法或安全合作、湄公河水資源分配與流域治理等議題，因同時涉及該外交官
轄下兩個國家，應列為優先關注項目，不宜僅視為單一國家的一般外交新聞。

【原則三：政治穩定度與體制動態】
兩國皆為一黨執政的社會主義體制，凡涉及越共或寮國人民革命黨最高領導人重要
言論、黨政人事異動、反貪腐與紀律查處案件、國會或國民議會立法動態等，均應
列入，作為研判兩國政局穩定度與權力結構變化的核心指標；涉及社會維穩、異議
管控等新聞亦應納入。

【原則四：經濟與產業戰略意涵】
凡涉及兩國總體經濟數據、外商直接投資動向（尤其台商佈局與供應鏈轉移至越南）、
能源與重大基礎建設政策等，均應列入。寮國部分應特別留意其對中國的高額外債與
經濟依賴度、中寮鐵路及水力發電輸出等議題，這是判斷寮國財政穩定度與對中戰略
自主性的關鍵面向；越南部分則應留意科技數位主權相關新聞，反映其在關鍵技術
供應鏈上的定位選擇。

【原則五：台灣連結與領務利益】
凡涉及在越南或寮國的台灣人（投資、觀光、僑界活動），或在台灣的越南人、寮國人
（移工、社群治理、犯罪、詐騙受害、跨境公衛事件等）之新聞，即使單獨來看屬地方
社會新聞層級，仍應一律列入，因涉及台灣直接的治理責任與領務保護意涵。

【原則六：公共安全、衛生與重大意外事件】
凡涉及越南、寮國天災應變整備、重大公共衛生事件、重大交通或旅遊安全意外等，
均應列入，尤其湄公河流域跨境事務（如洪災、乾旱、金三角毒品走私）應優先納入，
因這類議題往往同時牽動越南與寮國雙方，具跨境協調的迫切性。

【原則七：南海與湄公河區域安全】
聚焦越南、寮國本身直接涉入的區域安全事務：涉及越南在南海的主權主張、與周邊
聲索國的爭議動態，以及寮國邊境（尤其金三角地帶）毒品走私、跨境犯罪等安全
議題，均應列入；不涉及越南或寮國本身的其他區域地緣政治新聞，則不列為優先
關注範圍。

【原則八：國際形象與軟實力】（優先順序較低）
涉及越南、寮國重要體育賽事成績、文化交流活動、觀光排名等提升國際能見度的
新聞，可列入但優先順序較低，主要用於掌握對外形象經營與軟實力布局的輔助脈絡。

【原則九：例行治理與數位轉型】（優先順序通常最低）
涉及越南、寮國數位政府服務、行政規費調整等內政治理措施，優先順序通常最低，
除非涉及外資便利化、跨境事務或具指標性的治理模式轉型意涵。

【排除原則】
純屬娛樂八卦、不具公共政策意涵、且與越南、寮國或台灣無直接關聯的第三國瑣事
新聞，原則上不予列入。
"""
...
以下九項原則界定「應納入」的新聞範圍，請依每項原則判斷是否納入，並依下方
分類代碼對照決定歸入 01（政治）、02（經濟）或 03（其他）。同一原則下的新聞
可能因具體內容不同而分屬不同類別，對照僅供參考，仍須依新聞實際內容判斷。
若不確定是否符合任一原則，一律 discard（寧可漏收，不要多收）。

{PRIORITY_PRINCIPLES}

【分類代碼對照】
- 原則一（雙邊與多邊外交事務）→ 01 政治
- 原則二（越寮特殊關係與跨境治理）→ 高層互訪、司法安全合作等外交治理面向 → 01 政治；
  水資源分配與流域治理等跨境資源議題 → 03 其他
- 原則三（政治穩定度與體制動態）→ 01 政治
- 原則四（經濟與產業戰略意涵）→ 02 經濟
- 原則五（台灣連結與領務利益）→ 03 其他
- 原則六（公共安全、衛生與重大意外事件）→ 03 其他
- 原則七（南海與湄公河區域安全）→ 南海主權與爭議動態 → 01 政治；
  邊境毒品走私、跨境犯罪 → 03 其他
- 原則八（國際形象與軟實力）→ 03 其他
- 原則九（例行治理與數位轉型）→ 依內政或經濟性質判斷 → 01 政治或 02 經濟

【重要】上方九項原則列出的情境是「常見範例」，用來說明判斷的方向與尺度，
並非窮舉清單。國際局勢與新聞型態會持續演變，可能出現原則描述未直接列舉、
但同樣符合該原則精神的新型態事件，請理解每項原則「背後的判斷邏輯」並類推
套用，而不是機械式比對「是否完全符合列出的某一條」。
"""
...
參考價值，由「最重要」到「最不重要」排出完整順序。判斷依據是下列九項原則，
原則編號（一～九）本身即代表優先順序，編號越前面優先程度越高：

{PRIORITY_PRINCIPLES}

若一則新聞同時符合多項原則（例如「涉及台灣人的跨國詐騙案件」同時符合原則一
與原則二），應排在更前面。若新聞內容不屬於上述任一原則描述的類型，則排在
所有符合原則之新聞的後方，依原始順序處理即可。
